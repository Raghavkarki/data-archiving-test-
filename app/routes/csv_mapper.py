...
def process_csv():
    ...
    current_app.logger.info(f"CSV processing request - File: {file.filename}, IP: {request.remote_addr}")
    ...
    if filename.endswith(".csv"):
        ...
        current_app.logger.warning(f"Unsupported file format: {file.filename}")
    ...
    output.seek(0)
    current_app.logger.info(f"CSV processing completed - File: {file.filename}, Rows: {len(reordered_df)}, Output: {download_name}")
